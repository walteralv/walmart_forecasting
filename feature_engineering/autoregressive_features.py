...
def add_lags(
    ...
    Args:
        ...
        target (str): Name of the column to be lagged
        columns_id (list[str], optional): Column name of Unique ID of a time series to be grouped by before applying the lags.
    ...
    ), "`target` should be a valid column in the provided dataframe"
    _32_bit_dtype = _get_32_bit_dtype(df[target])
    df.sort_values(by=columns_id, ascending=True, inplace=True)
    columns_id.remove(time_column)
    lags = [0, ] + lags
    if use_32_bit and _32_bit_dtype is not None:
        col_dict = {
            f"{target}_lag_{l}": df.groupby(columns_id)[target]
            .shift(back_horizon + l)
            .astype(_32_bit_dtype)
            for l in tqdm(lags, desc="Adding memory variable 😸")
        }
    else:
        col_dict = {
            f"{target}_lag_{l}": df.groupby(columns_id)[target].shift(back_horizon + l) 
            for l in tqdm(lags, desc="Adding memory variable 😸")
        }
    columns_id.append(time_column)
...
def add_rolling_features(
    ...
    rolling_df = pd.concat(
        [
            df.groupby(columns_id)[target]
            .shift(back_horizon)
            .rolling(l)
            .agg({f"{target}_rolling_{back_horizon + l}_{agg}": agg for agg in tqdm(agg_funcs, desc="Adding rolling memory variable 😸")})
            for l in rolls
        ],
        axis=1,
    )
